(function () {
  ...
  function renderCalendar() {
    ...
    for (let index = 0; index < 42; index += 1) {
      ...
      appendUsageMarkers(button, dateKey);
  ...
  function appendUsageMarkers(dayButton, dateKey) {
    const expenses = state.expenses.filter((expense) => expense.date === dateKey);
    const directTotal = expenses
      .filter((expense) => expense.paymentMethod !== Core.CREDIT_PAYMENT)
      .reduce((total, expense) => total + Core.normalizeAmount(expense.amount), 0);

    if (directTotal > 0) {
      dayButton.append(createElement("span", "day-amount usage", formatYen(directTotal)));
    }
    ...
    expenses
      .filter((expense) => expense.paymentMethod === Core.CREDIT_PAYMENT)
      .forEach((expense) => {
        const key = expense.cardId || "unselected";
        cardTotals.set(key, (cardTotals.get(key) || 0) + Core.normalizeAmount(expense.amount));
      });

    cardTotals.forEach((amount, cardId) => {
